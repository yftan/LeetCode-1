...
public class Solution {
    ...
    public static boolean exist(char[][] board, String word) {
        for(int i=0; i < board.length;i++) {
            for(int j=0; j< board[0].length; j++) {
                isExist(board, word, 0, i,j, new boolean[board.length][board[0].length]);
    ...
    public static void isExist(char[][] board, String word, int pos, int i, int j, boolean[][] isVisited){
        if(pos == word.length()) {
            res =true;
            return;
        }
        if(i <0 || j<0 || i> board.length-1 || j>board[0].length-1 || pos >word.length()-1 || isVisited[i][j]) return;
        if(word.charAt(pos) != board[i][j]) return;
        ...
        // 这个判断条件不写会超时
        if(!res){
            isExist(board, word, pos +1 , i+1,j,isVisited);
            isExist(board, word, pos +1 , i-1,j,isVisited);
            isExist(board, word, pos +1 , i,j+1,isVisited);
            isExist(board, word, pos +1 , i,j-1,isVisited);
        }
        // 回溯法，要将状态恢复成原状态
        isVisited[i][j]= false;
